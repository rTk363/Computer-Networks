	...
	public static void main(String[] args) {
		...
		try {
			...
			while(true){
				...
				String[] request_array = strGreeting.split(" ");
				System.out.println("Client says = " + request_array[1]);

				// prepare data
				if(consignment+1 != secret.length) {
					sd= new String("RDT "+consignment+" "+secret[consignment]+" \r\n").getBytes();
				}
				else{
					sd= new String("RDT "+consignment+" "+secret[consignment]+" END"+" \r\n").getBytes();
				}
